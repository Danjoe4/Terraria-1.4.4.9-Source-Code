...
public class Mount
{
	...
	public IEntitySource GetProjectileSpawnSource(Player mountedPlayer) => new EntitySource_Mount(mountedPlayer, Type);
	...
	private Vector2 GetWitchBroomTrinketOriginOffset(Player player) => new Vector2(29 * player.direction, -4f);
	...
	public void UpdateFrame(Player mountedPlayer, int state, Vector2 velocity)
	{
		...
		switch (_type) {
			...
			case 45: {
				...
				if (flag) {
					int num15 = Utils.SelectRandom(Main.rand, new short[3] {
						6,
						6,
						31
					});
	...
	public bool CanMount(int m, Player mountingPlayer) => mountingPlayer.CanFitSpace(mounts[m].heightBoost);

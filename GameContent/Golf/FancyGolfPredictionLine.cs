...
public class FancyGolfPredictionLine
{
	...
	private float GetScale(float travelledLength) => 0.2f + Utils.GetLerpValue(0.8f, 1f, (float)Math.Cos(travelledLength / 50f + _time * -(float)Math.PI) * 0.5f + 0.5f, clamped: true) * 0.15f;
